...
import { messagesTotal, messageSize, wsConnections, } from '../utils/metrics.js';
export class WSHandler {
    ...
    config;
    constructor(wss, registry, config = {}, options) {
        this.wss = wss;
    ...
    handleStream(_socket, msg) {
        if (msg.reply_to) {
            console.log(`[WS] Stream chunk for ${msg.reply_to}`);
        }
    }
    handleEvent(_socket, msg) {
        const to = msg.to;
        if (to === 'broadcast') {
    ...
    handleJobSubmission(socket, _msg) {
        this.send(socket, {
            version: '1.0.0',
            msg_id: uuidv4(),
    ...
    handleCapabilityQuery(socket, msg) {
    ...
    sendError(socket, error, details) {
        this.send(socket, {
            version: '1.0.0',
            msg_id: uuidv4(),
            timestamp: new Date().toISOString(),
    ...
    broadcastToCapability(msg, capability, excludeAgentId) {
    ...
    startHeartbeat(socket, connectionId) {
