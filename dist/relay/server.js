...
export class RelayServer {
            ...
            socket.on('message', (data) => {
                try {
                    ...
                    if (raw.action === 'register') {
                        this.handleRegistration(socket, raw);
                        return;
                    }
    ...
    handleRegistration(socket, msg) {
        ...
        const agentInfo = {
            ...
            os: os || 'unknown',
            capabilities: capabilities?.map((c) => c.name || c) || [],
    ...
    handleJobSubmission(msg) {
        const { type, priority, payload, capabilityRequired, constraints } = msg.payload.data;
        const job = this.jobQueue.submit(msg.from.agent_id, type, priority ?? 5, payload, capabilityRequired, constraints);
        // Notify sender
