...
export class SHACLValidator {
     ...
     */
    parseTTL(content, shapeName) {
        const shape = { name: shapeName, fields: new Map() };
        ...
        for (const line of lines) {
            ...
            if (currentField) {
                ...
                if (minMatch)
                    currentField.minInclusive = parseInt(minMatch[1]);
                const maxMatch = trimmed.match(/sh:maxInclusive (\d+)/);
                if (maxMatch)
                    currentField.maxInclusive = parseInt(maxMatch[1]);
            }
            // Detect sh:datatype
            if (trimmed.includes('sh:datatype') && currentField) {
                if (trimmed.includes('xsd:string'))
                    currentField.datatype = 'string';
                else if (trimmed.includes('xsd:integer') || trimmed.includes('xsd:int'))
                    currentField.datatype = 'integer';
                else if (trimmed.includes('xsd:boolean'))
                    currentField.datatype = 'boolean';
                else if (trimmed.includes('xsd:dateTime'))
                    currentField.datatype = 'datetime';
                else if (trimmed.includes('xsd:anyURI'))
                    currentField.datatype = 'uri';
            }
            // Detect sh:pattern
            if (trimmed.includes('sh:pattern') && currentField) {
                const match = trimmed.match(/sh:pattern "([^"]+)"/);
                if (match)
                    currentField.pattern = new RegExp(match[1]);
            }
            // Detect sh:minLength
            if (trimmed.includes('sh:minLength') && currentField) {
                const match = trimmed.match(/sh:minLength (\d+)/);
                if (match)
                    currentField.minLength = parseInt(match[1]);
            }
            ...
            if (this._inShInBlock && currentField) {
                ...
                for (const m of matches)
                    this._currentShInValues.push(m[1]);
     ...
     */
    validateType(fieldName, value, datatype) {
        switch (datatype) {
            case 'string':
...
export function getValidator(shapesDir) {
